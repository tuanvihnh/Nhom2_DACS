    ...
    public partial class Forgot : System.Web.UI.Page
    {
        ...
        public void SendEmail(string to, string subject, string body, string imagePath)
        {
            try
            {
                ...
                string htmlBody = $"<html><body><img style=\"max-width: 100%;  border-radius:20px; \" src='cid:{inline.ContentId}' alt='Gmilk Tea' /><h1>Verify OTP</h1><p>Đây là mã OTP của bạn: <strong>{body}</strong>. Vui lòng không cung cấp mã này với bất kỳ ai!</p></body></html>";
        ...
        protected void btnGetOTP_Click(object sender, EventArgs e)
        {
            ...
            var user = db.TKs.FirstOrDefault(u => u.Email == email);
            string imagePath = Server.MapPath("Content\\image-bg\\Gmilk-Tea.png"); // Đường dẫn đến hình ảnh trên server

            if (user != null) SendEmail(user.Email, "OTP Gmilk Tea", otp, imagePath);
